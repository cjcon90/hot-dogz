...
class UploadForm(FlaskForm):
    name = StringField("Dog's name",
                       validators=[
                           DataRequired(),
                           Length(max=20,
                                  message="Max name length is 20 characters! "
    ...
    breed = SelectField("Breed",
                        choices=[
                            (breed.pk, breed.breed_name)
                            for breed in Breed.objects],
...
class EditForm(FlaskForm):
    ...
    breed = SelectField("Breed",
                        choices=[(breed.pk, breed.breed_name)
                                 for breed in Breed.objects],
                        validators=[DataRequired()])
    about = TextAreaField("Tell us about your dog!",
                          validators=[
...
class CommentInput(FlaskForm):
    content = TextAreaField(
        validators=[DataRequired('Please enter a comment first!'),
                    Length(max=250, message="Comments have a max character "
                                            "limit of 250!")])
    submit = SubmitField('Submit')
